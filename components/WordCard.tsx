...
import { fetchWordAudioBuffer, getWordExamples, getValidApiKey } from '../services/geminiService';
...
const WordCard: React.FC<WordCardProps> = ({ 
  ...
  const [isUsingNativeSpeech, setIsUsingNativeSpeech] = useState(false);
  ...
  const playWithNativeSpeech = (text: string, onEnd: () => void) => {
          ...
          <div className="max-w-xl mx-auto space-y-8">
            ...
            <section>
              <div className="flex items-center justify-between mb-3 px-1">
                ...
                <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full border uppercase tracking-tighter ${isUsingNativeSpeech ? 'bg-slate-100 text-slate-500 border-slate-200' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                  {isUsingNativeSpeech ? 'Offline Mode' : 'AI Powered'}
                </span>
              </div>
